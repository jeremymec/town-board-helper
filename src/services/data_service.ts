...
import { get_item_from_name, ItemModel } from "./item_service";
...
function compute_category_data(): CategoryModel[] {
  return category_data_raw.map((category) => {
    return {
      id: category.id,
      name: category.name,
      items: item_data.filter(
        (item) =>
          item.categories.includes(category.name) ||
          item.categories.includes(`${category.name} (PRIMARY)`)
      ),
    };
  });
}
  ...
  return recipe_data_raw.map((data) => ({
    ...
    components: [...Array.from(new Set(data.components))].map((component) => {
      return {
        ...
        quantity: data.components.filter(
          (otherComponent) => otherComponent === component
        ).length,
      };
    }),
    category_components: [...Array.from(new Set(data.category_components))].map(
      (category_component) => {
        return {
          category: get_category_from_name(category_component),
          quantity: data.category_components.filter(
            (otherCategoryComponent) =>
              otherCategoryComponent === category_component
          ).length,
        };
      }
    ),
